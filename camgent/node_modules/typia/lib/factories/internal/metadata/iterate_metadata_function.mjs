import { FunctionalGeneralProgrammer } from '../../../programmers/functional/internal/FunctionalGeneralProgrammer.mjs';
import { Metadata } from '../../../schemas/metadata/Metadata.mjs';
import { MetadataFunction } from '../../../schemas/metadata/MetadataFunction.mjs';
import { MetadataParameter } from '../../../schemas/metadata/MetadataParameter.mjs';
...
const iterate_metadata_function = (props) => {
    const declaration = TypeFactory.getFunction(props.type);
    if (declaration === null)
        return false;
    else if (!props.options.functional) {
        if (props.metadata.functions.length === 0)
            props.metadata.functions.push(MetadataFunction.create({
                parameters: [],
        ...
        if (signature === undefined || signature.declaration === undefined)
            ...
            props.metadata.functions.push(MetadataFunction.create({
                parameters: signature.parameters.map((p) => MetadataParameter.create({
                    name: p.name,
                ...
                output: explore_metadata({
                    ...
                    type: async
                        ? (props.checker.getTypeArguments(signature.getReturnType())?.[0] ??
...
export { iterate_metadata_function };
//# sourceMappingURL=iterate_metadata_function.mjs.map
