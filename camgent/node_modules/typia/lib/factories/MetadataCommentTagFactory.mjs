...
import { FormatCheatSheet } from '../tags/internal/FormatCheatSheet.mjs';
import { MetadataTypeTagFactory } from './MetadataTypeTagFactory.mjs';
...
(function (MetadataCommentTagFactory) {
            ...
            for (const [key, value] of Object.entries(tagger)) {
                ...
                if (key === "array") {
                    ...
                    for (const a of props.metadata.arrays) {
                        Writable(a).tags = a.tags.filter((x) => MetadataTypeTagFactory.validate({
                    ...
                    if (atomic === undefined)
                        ...
                        Writable(atomic).tags = atomic.tags.filter((x) => MetadataTypeTagFactory.validate({
                            report: validateReport,
                            type: key,
                            tags: [...x, ...filtered],
                        }));
    ...
    MetadataCommentTagFactory.get = (props) => {
        const output = PARSER[props.kind]?.({
            report: () => null,
            value: props.value,
        })?.[props.type];
        if (output === undefined)
...
const FORMATS = new Map([
    ...Object.entries(FormatCheatSheet).map(([key, value]) => [key, [key, value]]),
    ["datetime", ["date-time", `!isNaN(new Date($input).getTime())`]],
    ["dateTime", ["date-time", `!isNaN(new Date($input).getTime())`]],
]);

export { MetadataCommentTagFactory };
//# sourceMappingURL=MetadataCommentTagFactory.mjs.map
