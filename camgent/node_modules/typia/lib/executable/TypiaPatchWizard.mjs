        ...
        for (const file of ["tsc.js", "_tsc.js"])
            try {
                ...
                if (content.indexOf(FROM_WITH_COMMENT) !== -1)
                    await fs.promises.writeFile(location, content.replace(FROM_WITH_COMMENT, TO_WITH_COMMENT), "utf8");
                else if (content.indexOf(FROM_ONLY) !== -1)
                    await fs.promises.writeFile(location, content.replace(FROM_ONLY, TO_ONLY), "utf8");
            }
            catch { }
    };
