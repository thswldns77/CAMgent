...
import { check_dynamic_key } from './check_dynamic_key.mjs';
import { check_everything } from './check_everything.mjs';

/** @internal */
const check_dynamic_properties = (props) => {
    const length = IdentifierFactory.access(ts.factory.createCallExpression(ts.factory.createIdentifier("Object.keys"), undefined, [props.input]), "length");
    const left = props.config.equals === true && props.dynamic.length === 0
        ? props.config.undefined === true ||
            ...
            ? ts.factory.createStrictEquality(ExpressionFactory.number(props.regular.filter((r) => r.meta.isRequired()).length), length)
            : ts.factory.createCallExpression(props.context.importer.internal("isBetween"), [], [
                length,
                ExpressionFactory.number(props.regular.filter((r) => r.meta.isRequired()).length),
    ...
    const criteria = props.config.entries
        ? ts.factory.createCallExpression(props.config.entries, undefined, [
            ts.factory.createCallExpression(ts.factory.createIdentifier("Object.keys"), undefined, [props.input]),
            check_dynamic_property(props),
        ])
        : ts.factory.createCallExpression(IdentifierFactory.access(ts.factory.createCallExpression(ts.factory.createIdentifier("Object.keys"), undefined, [props.input]), props.config.assert ? "every" : "map"), undefined, [check_dynamic_property(props)]);
    const right = (props.config.halt || ((elem) => elem))(props.config.assert ? criteria : check_everything(criteria));
    return left
...
const check_dynamic_property = (props) => {
    ...
    const statements = [];
    const add = (expression, output) => statements.push(ts.factory.createIfStatement(expression, ts.factory.createReturnStatement(output)));
    ...
    if (props.config.undefined === true)
        add(ts.factory.createStrictEquality(ts.factory.createIdentifier("undefined"), value), props.config.positive);
    for (const entry of props.dynamic) {
        const condition = check_dynamic_key({
