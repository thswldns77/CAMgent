...
import { CloneJoiner } from '../helpers/CloneJoiner.mjs';
...
(function (MiscCloneProgrammer) {
    ...
    MiscCloneProgrammer.write = (props) => {
        ...
        const result = MiscCloneProgrammer.decompose({
            ...props,
            validated: false,
            functor,
        });
    ...
    const decode = (props) => {
        ...
        if (props.metadata.any ||
            ...
            return ts.factory.createCallExpression(props.context.importer.internal("miscCloneAny"), undefined, [props.input]);
        const unions = [];
        //----
        // LIST UP UNION TYPES
        //----
    ...
    const decode_native = (props) => props.type === "Date" ||
        ...
        : props.type === "ArrayBuffer" || props.type === "SharedArrayBuffer"
            ...
            : props.type === "DataView"
                ...
                : ts.factory.createCallExpression(ts.factory.createIdentifier(props.type), undefined, []);
    const decode_native_copyable = (props) => ts.factory.createNewExpression(ts.factory.createIdentifier(props.type), undefined, [props.input]);
    const decode_native_buffer = (props) => ExpressionFactory.selfCall(ts.factory.createBlock([
        StatementFactory.constant({
            name: "buffer",
            value: ts.factory.createNewExpression(ts.factory.createIdentifier(props.type), undefined, [IdentifierFactory.access(props.input, "byteLength")]),
        }),
        ts.factory.createExpressionStatement(ts.factory.createCallExpression(IdentifierFactory.access(ts.factory.createNewExpression(ts.factory.createIdentifier("Uint8Array"), undefined, [ts.factory.createIdentifier("buffer")]), "set"), undefined, [
            ts.factory.createNewExpression(ts.factory.createIdentifier("Uint8Array"), undefined, [props.input]),
        ])),
        ts.factory.createReturnStatement(ts.factory.createIdentifier("buffer")),
    ], true));
    const decode_native_data_view = (input) => ts.factory.createNewExpression(ts.factory.createIdentifier("DataView"), undefined, [IdentifierFactory.access(input, "buffer")]);
    /* -----------------------------------------------------------
