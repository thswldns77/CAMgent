...
import { check_everything } from './internal/check_everything.mjs';
...
(function (ValidateProgrammer) {
    ValidateProgrammer.decompose = (props) => {
        const is = IsProgrammer.decompose(props);
                ...
                atomist: (next) => [
                    ...
                    ...(next.entry.conditions.length === 0
                        ...
                        : next.entry.conditions.length === 1
                            ? next.entry.conditions[0].map((cond) => ts.factory.createLogicalOr(cond.expression, create_report_call({
                            ...
                            : [
                                ts.factory.createLogicalOr(next.entry.conditions
                                    ...
                                    .reduce((a, b) => ts.factory.createLogicalOr(a, b)), create_report_call({
                                    exceptionable: next.explore.from === "top"
                                        ? ts.factory.createTrue()
                                        : ts.factory.createIdentifier("_exceptionable"),
                                    path: ts.factory.createIdentifier(next.explore.postfix
        ...
        const arrow = ts.factory.createArrowFunction(undefined, undefined, [IdentifierFactory.parameter("input", TypeFactory.keyword("any"))], props.context.importer.type({
            file: "typia",
            name: "IValidation",
            arguments: [
            ...
            ts.factory.createIfStatement(ts.factory.createStrictEquality(ts.factory.createFalse(), ts.factory.createCallExpression(ts.factory.createIdentifier("__is"), undefined, [ts.factory.createIdentifier("input")])), ts.factory.createBlock([
                // prepare errors
                ts.factory.createExpressionStatement(ts.factory.createBinaryExpression(ts.factory.createIdentifier("errors"), ts.factory.createToken(ts.SyntaxKind.EqualsToken), ts.factory.createArrayLiteralExpression([]))),
                ts.factory.createExpressionStatement(ts.factory.createBinaryExpression(ts.factory.createIdentifier("_report"), ts.factory.createToken(ts.SyntaxKind.EqualsToken), ts.factory.createCallExpression(ts.factory.createAsExpression(props.context.importer.internal("validateReport"), TypeFactory.keyword("any")), [], [ts.factory.createIdentifier("errors")]))),
                ...
                StatementFactory.constant({
                    name: "success",
                    value: ts.factory.createStrictEquality(ExpressionFactory.number(0), ts.factory.createIdentifier("errors.length")),
                }),
    ...
    ValidateProgrammer.write = (props) => {
        ...
        return FeatureProgrammer.writeDecomposed({
            ...
            returnWrapper: props.config.standardSchema
                ? (arrow) => ts.factory.createCallExpression(props.context.importer.internal("createStandardSchema"), undefined, [arrow])
                : undefined,
        });
    };
