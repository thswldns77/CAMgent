...
class UniqueSet extends SetContainer {
    ...
    _Insert_by_range(first, last) {
        for (;!first.equals(last); first = first.next()) this._Insert_by_key(first.value);
    }
    extract(param) {
...
class UniqueMap extends MapContainer {
    ...
    _Insert_by_range(first, last) {
        for (let it = first; !it.equals(last); it = it.next()) this.emplace(it.value.first, it.value.second);
    }
    ...
    merge(source) {
        for (let it = source.begin(); !it.equals(source.end()); ) if (this.has(it.first) === false) {
            this.insert(it.value);
            it = source.erase(it);
        } else it = it.next();
    }
...
function is_node() {
    if (is_node_ === null) is_node_ = typeof global === "object" && is_node_process(global);
    return is_node_;
}
...
function get_uid(obj) {
    ...
    } else if (obj === undefined) return -1; else return 0;
}
...
function less(x, y) {
    x = x.valueOf();
    y = y.valueOf();
    if (x instanceof Object) if (x.less instanceof Function) return x.less(y); else return get_uid(x) < get_uid(y); else return x < y;
}
...
(function(ITreeContainer) {
    ...
    function emplacable(source, hint, elem) {
        ...
        let meet = prev.equals(source.end()) || source.value_comp()(prev.value, elem);
...
function hash(...itemList) {
    ...
    for (let item of itemList) {
        ...
        if (type === "boolean") ret = _Hash_boolean(item, ret); else if (type === "number" || type === "bigint") ret = _Hash_number(item, ret); else if (type === "string") ret = _Hash_string(item, ret); else if (item instanceof Object && item.hashCode instanceof Function) {
...
class UniqueTreeSet extends UniqueSet {
    ...
    _Insert_by_key(key) {
        ...
        if (!it.equals(this.end()) && this._Key_eq(it.value, key)) return new Pair(it, false);
...
class ArrayContainer extends Container {
    ...
    insert(pos, ...args) {
        if (pos._Get_array() !== this) throw ErrorGenerator.not_my_iterator(this.source(), "insert"); else if (pos.index() < 0) throw ErrorGenerator.negative_iterator(this.source(), "insert", pos.index()); else if (pos.index() > this.size()) pos = this.end();
        if (args.length === 1) return this._Insert_by_repeating_val(pos, 1, args[0]); else if (args.length === 2 && typeof args[0] === "number") return this._Insert_by_repeating_val(pos, args[0], args[1]); else return this._Insert_by_range(pos, args[0], args[1]);
    }
...
class VectorContainer extends ArrayContainer {
    ...
    _Insert_by_range(position, first, last) {
        if (position.index() >= this.size()) {
            ...
            for (;!first.equals(last); first = first.next()) this.data_.push(first.value);
...
class FlatSet extends UniqueTreeSet {
    constructor(...args) {
        ...
        ITreeContainer.construct(this, FlatSet, (comp => {
            this.key_comp_ = comp;
        }), ...args);
    }
    swap(obj) {
...
class MultiTreeSet extends MultiSet {
    ...
    count(key) {
        ...
        let ret = 0;
        for (;!it.equals(this.end()) && this._Key_eq(it.value, key); it = it.next()) ++ret;
        return ret;
    }
    equal_range(key) {
    ...
    _Insert_by_range(first, last) {
        for (let it = first; !it.equals(last); it = it.next()) this._Insert_by_key(it.value);
    }
}
...
class FlatMultiSet extends MultiTreeSet {
    constructor(...args) {
        super((thisArg => new SetElementVector(thisArg)));
        ITreeContainer.construct(this, FlatMultiSet, (comp => {
            this.key_comp_ = comp;
        }), ...args);
    }
    swap(obj) {
...
class UniqueTreeMap extends UniqueMap {
    ...
    emplace(key, val) {
        let it = this.lower_bound(key);
        if (!it.equals(this.end()) && this._Key_eq(it.first, key)) return new Pair(it, false);
...
class FlatMap extends UniqueTreeMap {
    constructor(...args) {
        ...
        ITreeContainer.construct(this, FlatMap, (comp => {
            this.key_comp_ = comp;
        }), ...args);
    }
    swap(obj) {
    ...
    lower_bound(key) {
        return lower_bound$1(this.begin(), this.end(), this._Capsule_key(key), this.value_comp());
    }
    upper_bound(key) {
        return upper_bound$1(this.begin(), this.end(), this._Capsule_key(key), this.value_comp());
    }
    _Capsule_key(key) {
        return {
            first: key
...
class MultiTreeMap extends MultiMap {
    ...
    count(key) {
        let it = this.find(key);
        let ret = 0;
        for (;!it.equals(this.end()) && this._Key_eq(it.first, key); it = it.next()) ++ret;
        return ret;
    }
    equal_range(key) {
        return new Pair(this.lower_bound(key), this.upper_bound(key));
...
class FlatMultiMap extends MultiTreeMap {
    constructor(...args) {
        super((thisArg => new MapElementVector(thisArg)));
        ITreeContainer.construct(this, FlatMultiMap, (comp => {
            this.key_comp_ = comp;
        }), ...args);
    }
    swap(obj) {
...
function is_heap_until$1(first, last, comp = less) {
    let counter = 0;
    for (let child = first.next(); _Comp_it(child, last.index()); child = child.next(), 
    counter ^= 1) {
...
function sort_heap$1(first, last, comp = less) {
    for (;distance(first, last) > 1; last = last.prev()) pop_heap$1(first, last, comp);
}

function _Promote_heap(first, topPosition, position, value, comp) {
    for (let parentPosition = position - 1 >> 1; position > topPosition && comp(first.advance(parentPosition).value, value); parentPosition = position - 1 >> 1) {
...
function _Adjust_heap(first, topPosition, heapSize, position, value, comp) {
    ...
    for (;childPosition < heapSize; childPosition = 2 * childPosition + 2) {
        if (comp(first.advance(childPosition).value, first.advance(childPosition - 1).value)) --childPosition;
...
function _Comp_it(x, y) {
    if (x.base instanceof Function) return y < x; else return x < y;
}
...
function all_of$1(first, last, pred) {
    for (let it = first; !it.equals(last); it = it.next()) if (pred(it.value) === false) return false;
    return true;
}

function any_of$1(first, last, pred) {
    for (let it = first; !it.equals(last); it = it.next()) if (pred(it.value) === true) return true;
    return false;
}
...
function equal$1(first1, last1, first2, pred = equal_to) {
    while (!first1.equals(last1)) if (!pred(first1.value, first2.value)) return false; else {
        first1 = first1.next();
        first2 = first2.next();
    }
    return true;
...
function lexicographical_compare$1(first1, last1, first2, last2, comp = less) {
    while (!first1.equals(last1)) if (first2.equals(last2) || comp(first2.value, first1.value)) return false; else if (comp(first1.value, first2.value)) return true; else {
        first1 = first1.next();
        first2 = first2.next();
    }
...
function find_if$1(first, last, pred) {
    for (let it = first; !it.equals(last); it = it.next()) if (pred(it.value)) return it;
    return last;
}
...
function find_first_of$1(first1, last1, first2, last2, pred = equal_to) {
    for (;!first1.equals(last1); first1 = first1.next()) for (let it = first2; !it.equals(last2); it = it.next()) if (pred(first1.value, it.value)) return first1;
    return last1;
}
...
function search$1(first1, last1, first2, last2, pred = equal_to) {
    ...
    for (;!first1.equals(last1); first1 = first1.next()) {
        ...
        let it2 = first2;
        while (pred(it1.value, it2.value)) {
            if (it2.equals(last2)) return first1; else if (it1.equals(last1)) return last1;
            it1 = it1.next();
            it2 = it2.next();
        }
    }
...
function equal(range1, range2, pred = equal_to) {
    if (size(range1) !== size(range2)) return false; else return equal$1(begin(range1), end(range1), begin(range2), pred);
}

function lexicographical_compare(range1, range2, comp = less) {
    return lexicographical_compare$1(begin(range1), end(range1), begin(range2), end(range2), comp);
}
...
function find_end(range1, range2, pred = equal_to) {
    return find_end$1(begin(range1), end(range1), begin(range2), end(range2), pred);
}

function find_first_of(range1, range2, pred = equal_to) {
    return find_first_of$1(begin(range1), end(range1), begin(range2), end(range2), pred);
}
...
function search(range1, range2, pred = equal_to) {
    return search$1(begin(range1), end(range1), begin(range2), end(range2), pred);
}
...
function mismatch(range1, range2, pred = equal_to) {
    if (size(range1) === size(range2)) return mismatch$1(begin(range1), end(range1), begin(range2), pred);
...
function partial_sort$1(first, middle, last, comp = less) {
    for (let i = first; !i.equals(middle); i = i.next()) {
        let min = i;
        for (let j = i.next(); !j.equals(last); j = j.next()) if (comp(j.value, min.value)) min = j;
        if (!i.equals(min)) iter_swap(i, min);
    }
}

function partial_sort_copy$1(first, last, output_first, output_last, comp = less) {
    ...
    if (input_size > result_size) output_first = copy$1(vector.begin(), vector.begin().advance(result_size), output_first); else output_first = copy$1(vector.begin(), vector.end(), output_first);
    return output_first;
}
...
function nth_element$1(first, nth, last, comp = less) {
    ...
    for (let i = first; !i.equals(last); i = i.next()) {
        let count = 0;
        for (let j = first; !j.equals(last); j = j.next()) if (i.equals(j)) continue; else if (comp(i.value, j.value) && ++count > n) break;
        if (count === n) {
            iter_swap(nth, i);
            return;
        }
...
function is_sorted_until$1(first, last, comp = less) {
    ...
    for (let next = first.next(); !next.equals(last); next = next.next()) if (comp(next.value, first.value)) return next; else first = first.next();
    return last;
}
...
function sample$1(first, last, output, n) {
    ...
    for (let i = n - 1; i >= 1; --i) advances.set(i, advances.at(i) - advances.at(i - 1));
...
function transform$1(...args) {
    if (args.length === 4) return _Unary_transform(...args); else return _Binary_transform(...args);
}
...
function unique$1(first, last, pred = equal_to) {
    if (first.equals(last)) return last;
    let ret = first;
    for (first = first.next(); !first.equals(last); first = first.next()) if (!pred(ret.value, first.value)) {
        ret = ret.next();
...
function unique_copy$1(first, last, output, pred = equal_to) {
    ...
    output.value = first.value;
    first = first.next();
    for (;!first.equals(last); first = first.next()) if (!pred(first.value, output.value)) {
        output = output.next();
        output.value = first.value;
    }
    return output.next();
...
function replace$1(first, last, old_val, new_val) {
    return replace_if$1(first, last, (elem => equal_to(elem, old_val)), new_val);
}
...
function replace_copy$1(first, last, output, old_val, new_val) {
    return replace_copy_if$1(first, last, output, (elem => equal_to(elem, old_val)), new_val);
}

function replace_copy_if$1(first, last, result, pred, new_val) {
    for (;!first.equals(last); first = first.next()) {
        if (pred(first.value)) result.value = new_val; else result.value = first.value;
        result = result.next();
    }
    return result;
}
...
function min(items, comp = less) {
    let minimum = items[0];
    for (let i = 1; i < items.length; ++i) if (comp(items[i], minimum)) minimum = items[i];
    return minimum;
}

function max(items, comp = less) {
    let maximum = items[0];
    for (let i = 1; i < items.length; ++i) if (comp(maximum, items[i])) maximum = items[i];
    return maximum;
}
...
function min_element$1(first, last, comp = less) {
    let smallest = first;
    first = first.next();
    for (;!first.equals(last); first = first.next()) if (comp(first.value, smallest.value)) smallest = first;
    return smallest;
}
...
function max_element$1(first, last, comp = less) {
    let largest = first;
    first = first.next();
    for (;!first.equals(last); first = first.next()) if (comp(largest.value, first.value)) largest = first;
    return largest;
}
...
function is_permutation(range1, range2, pred = equal_to) {
    if (size(range1) !== size(range2)) return false; else return is_permutation$1(begin(range1), end(range1), begin(range2), pred);
}
...
function transform(range1, ...args) {
    ...
    if (args.length === 3) return fn(...args); else return fn(end(args[1]), args[2], args[3]);
}
...
function is_partitioned$1(first, last, pred) {
    ...
    for (;!first.equals(last); first = first.next()) if (pred(first.value)) return false;
    return true;
}
...
function partition_copy$1(first, last, output_true, output_false, pred) {
    for (;!first.equals(last); first = first.next()) if (pred(first.value)) {
...
function partition_copy(range, output_true, output_false, pred) {
    return partition_copy$1(begin(range), end(range), output_true, output_false, pred);
}
...
function partial_sort_copy(range, output, comp = less) {
    return partial_sort_copy$1(begin(range), end(range), begin(output), end(output), comp);
}
...
function merge$1(first1, last1, first2, last2, output, comp = less) {
    while (true) {
        if (first1.equals(last1)) return copy$1(first2, last2, output); else if (first2.equals(last2)) return copy$1(first1, last1, output);
...
function includes$1(first1, last1, first2, last2, comp = less) {
    while (!first2.equals(last2)) {
        if (first1.equals(last1) || comp(first2.value, first1.value)) return false; else if (!comp(first1.value, first2.value)) first2 = first2.next();
        first1 = first1.next();
    }
    return true;
}
...
function set_intersection$1(first1, last1, first2, last2, output, comp = less) {
    while (!first1.equals(last1) && !first2.equals(last2)) if (comp(first1.value, first2.value)) first1 = first1.next(); else if (comp(first2.value, first1.value)) first2 = first2.next(); else {
        output.value = first1.value;
        output = output.next();
        first1 = first1.next();
        first2 = first2.next();
...
function set_difference$1(first1, last1, first2, last2, output, comp = less) {
    while (!first1.equals(last1) && !first2.equals(last2)) if (comp(first1.value, first2.value)) {
        output.value = first1.value;
        output = output.next();
        first1 = first1.next();
    } else if (comp(first2.value, first1.value)) first2 = first2.next(); else {
        first1 = first1.next();
        first2 = first2.next();
    }
...
function set_symmetric_difference$1(first1, last1, first2, last2, output, comp = less) {
    while (true) {
        if (first1.equals(last1)) return copy$1(first2, last2, output); else if (first2.equals(last2)) return copy$1(first1, last1, output);
        if (comp(first1.value, first2.value)) {
            output.value = first1.value;
...
function merge(range1, range2, output, comp = less) {
    return merge$1(begin(range1), end(range1), begin(range2), end(range2), output, comp);
}
...
function includes(range1, range2, comp = less) {
    if (size(range1) < size(range2)) return false; else return includes$1(begin(range1), end(range1), begin(range2), end(range2), comp);
}

function set_union(range1, range2, output, comp = less) {
    return set_union$1(begin(range1), end(range1), begin(range2), end(range2), output, comp);
}

function set_intersection(range1, range2, output, comp = less) {
    return set_intersection$1(begin(range1), end(range1), begin(range2), end(range2), output, comp);
}

function set_difference(range1, range2, output, comp = less) {
    return set_difference$1(begin(range1), end(range1), begin(range2), end(range2), output, comp);
}

function set_symmetric_difference(range1, range2, output, comp = less) {
    return set_symmetric_difference$1(begin(range1), end(range1), begin(range2), end(range2), output, comp);
}

function plus(x, y) {
    if (x.plus instanceof Function) return x.plus(y); else return x + y;
}

function minus(x, y) {
    if (x.minus instanceof Function) return x.minus(y); else return x - y;
}

function negate(x) {
    if (x.negate instanceof Function) return x.negate(); else return -x;
}

function multiplies(x, y) {
    if (x.multiplies instanceof Function) return x.multiplies(y); else return x * y;
}

function divides(x, y) {
    if (x.divides instanceof Function) return x.divides(y); else return x / y;
}

function modules(x, y) {
    if (x.modules instanceof Function) return x.modules(y); else return x % y;
}
...
function accumulate$1(first, last, init, op = plus) {
    for (;!first.equals(last); first = first.next()) init = op(init, first.value);
    return init;
}

function inner_product$1(first1, last1, first2, value, adder = plus, multiplier = multiplies) {
    for (;!first1.equals(last1); first1 = first1.next()) {
...
function inclusive_scan$1(first, last, output, adder = plus, init) {
    return transform_inclusive_scan$1(first, last, output, adder, (val => val), init);
}

function exclusive_scan$1(first, last, output, init, op = plus) {
    return transform_exclusive_scan$1(first, last, output, init, op, (val => val));
}

function transform_inclusive_scan$1(first, last, output, binary, unary, init) {
    if (first.equals(last)) return output;
    let before;
...
function transform_exclusive_scan$1(first, last, output, init, binary, unary) {
    if (first.equals(last)) return output;
...
function inner_product(range1, range2, value, adder = plus, multiplier = multiplies) {
    return inner_product$1(begin(range1), end(range1), begin(range2), value, adder, multiplier);
}
...
function transform_inclusive_scan(range, output, binary, unary, init) {
    return transform_inclusive_scan$1(begin(range), end(range), output, binary, unary, init);
}

function transform_exclusive_scan(range, output, init, binary, unary) {
    return transform_exclusive_scan$1(begin(range), end(range), output, init, binary, unary);
}
...
class Deque extends ArrayContainer {
    ...
    resize(n) {
        ...
        if (expansion > 0) this.insert(this.end(), expansion, undefined); else if (expansion < 0) this.erase(this.end().advance(-expansion), this.end());
    }
    reserve(n) {
    ...
    static _Emend(n, method) {
        n = Math.floor(n);
        if (n <= 0) throw new InvalidArgument(`Error on Deque.${method}(): n must be positive integer -> (n = ${n})`);
        return n;
    }
    size() {
        return this.size_;
    ...
    pop_front() {
        if (this.empty() === true) throw ErrorGenerator.empty(this.constructor, "pop_front");
        this.matrix_[0].shift();
        if (this.matrix_[0].length === 0 && this.matrix_.length > 1) this.matrix_.shift();
        this.size_--;
    }
    _Pop_back() {
    ...
    _Erase_by_range(first, last) {
        if (first.index() >= this.size()) return first;
        let size;
        if (last.index() >= this.size()) size = this.size() - first.index(); else size = last.index() - first.index();
        this.size_ -= size;
        let first_row = null;
        let second_row = null;
        let i = 0;
        while (size !== 0) {
            ...
            if (row.length !== 0) if (i === 0) first_row = row; else second_row = row;
            if (row.length === 0 && this.matrix_.length > 1) this.matrix_.splice(indexes.first, 1);
            size -= my_delete_size;
            ++i;
        }
        if (first_row !== null && second_row !== null && first_row.length + second_row.length <= this._Compute_col_size()) {
...
class ListContainer extends Container {
    ...
    insert(pos, ...args) {
        if (pos.source() !== this.end_.source()) throw ErrorGenerator.not_my_iterator(this.end_.source(), "insert"); else if (pos.erased_ === true) throw ErrorGenerator.erased_iterator(this.end_.source(), "insert");
        if (args.length === 1) return this._Insert_by_repeating_val(pos, 1, args[0]); else if (args.length === 2 && typeof args[0] === "number") return this._Insert_by_repeating_val(pos, args[0], args[1]); else return this._Insert_by_range(pos, args[0], args[1]);
    }
    _Insert_by_repeating_val(position, n, val) {
        const first = new Repeater(0, val);
    ...
    _Erase_by_range(first, last) {
        if (first.source() !== this.end_.source()) throw ErrorGenerator.not_my_iterator(this.end_.source(), "insert"); else if (first.erased_ === true) throw ErrorGenerator.erased_iterator(this.end_.source(), "insert"); else if (first.equals(this.end_)) return this.end_;
...
class List extends ListContainer {
    constructor(...args) {
        ...
        if (args.length === 0) ; else if (args.length === 1 && args[0] instanceof Array) {
            const array = args[0];
            this.push(...array);
    ...
    front(val) {
        if (arguments.length === 0) return this.begin_.value; else this.begin_.value = val;
    }
    back(val) {
        const it = this.end().prev();
        if (arguments.length === 0) return it.value; else it.value = val;
    }
    unique(binary_pred = equal_to) {
        let it = this.begin().next();
        while (!it.equals(this.end())) {
            if (binary_pred(it.value, it.prev().value) === true) it = this.erase(it); else it = it.next();
        }
    }
    remove(val) {
    ...
    remove_if(pred) {
        let it = this.begin();
        while (!it.equals(this.end())) {
            if (pred(it.value) === true) it = this.erase(it); else it = it.next();
        }
    }
    merge(source, comp = less) {
        ...
        while (source.empty() === false) {
            ...
            while (!it.equals(this.end()) && comp(it.value, first.value) === true) it = it.next();
    ...
    _Quick_sort_partition(first, last, comp) {
        ...
        let prev = first.prev();
        let it = first;
        for (;!it.equals(last); it = it.next()) if (comp(it.value, standard)) {
...
class XTree {
    ...
    nearest(val) {
        ...
        while (true) {
            let my_node = null;
            if (this.comp_(val, ret.value)) my_node = ret.left; else if (this.comp_(ret.value, val)) my_node = ret.right; else return ret;
    ...
    _Insert_case1(n) {
        if (n.parent === null) n.color = 0; else this._Insert_case2(n);
    }
    _Insert_case2(n) {
        if (this._Fetch_color(n.parent) === 0) return; else this._Insert_case3(n);
    }
    _Insert_case3(n) {
    ...
    _Erase_case3(n) {
        if (this._Fetch_color(n.parent) === 0 && this._Fetch_color(n.sibling) === 0 && this._Fetch_color(n.sibling.left) === 0 && this._Fetch_color(n.sibling.right) === 0) {
            n.sibling.color = 1;
            this._Erase_case1(n.parent);
        } else this._Erase_case4(n);
    }
    _Erase_case4(N) {
        if (this._Fetch_color(N.parent) === 1 && N.sibling !== null && this._Fetch_color(N.sibling) === 0 && this._Fetch_color(N.sibling.left) === 0 && this._Fetch_color(N.sibling.right) === 0) {
            N.sibling.color = 1;
            N.parent.color = 0;
        } else this._Erase_case5(N);
    }
    _Erase_case5(n) {
        if (n === n.parent.left && n.sibling !== null && this._Fetch_color(n.sibling) === 0 && this._Fetch_color(n.sibling.left) === 1 && this._Fetch_color(n.sibling.right) === 0) {
            n.sibling.color = 1;
            n.sibling.left.color = 0;
            this._Rotate_right(n.sibling);
        } else if (n === n.parent.right && n.sibling !== null && this._Fetch_color(n.sibling) === 0 && this._Fetch_color(n.sibling.left) === 0 && this._Fetch_color(n.sibling.right) === 1) {
            n.sibling.color = 1;
    ...
    _Fetch_color(node) {
        if (node === null) return 0; else return node.color;
    }
}
...
class MapTree extends XTree {
    ...
    get_by_key(key) {
        ...
        if (ret === null || !this.key_eq_(key, ret.value.first)) return null; else return ret;
    }
    lower_bound(key) {
        const node = this.nearest_by_key(key);
        if (node === null) return this.source().end(); else if (this.key_comp()(node.value.first, key)) return node.value.next(); else return node.value;
    }
    equal_range(key) {
        return new Pair(this.lower_bound(key), this.upper_bound(key));
    }
...
class UniqueMapTree extends MapTree {
    ...
    nearest_by_key(key) {
        ...
        while (true) {
            const it = ret.value;
            let my_node = null;
            if (this.key_comp()(key, it.first)) my_node = ret.left; else if (this.key_comp()(it.first, key)) my_node = ret.right; else return ret;
    ...
    upper_bound(key) {
        ...
        const it = node.value;
        if (this.key_comp()(key, it.first)) return it; else return it.next();
    }
}
...
class TreeMap extends UniqueTreeMap {
    constructor(...args) {
        ...
        ITreeContainer.construct(this, TreeMap, (comp => {
...
class VectorBoolean extends ArrayContainer {
    ...
    resize(n) {
        const expansion = n - this.size();
        if (expansion > 0) this.insert(this.end(), expansion, false); else if (expansion < 0) this.erase(this.end().advance(-expansion), this.end());
    }
    flip() {
    ...
    _Set(index, val) {
        ...
        const prev = it.prev();
        const next = it.next();
        if (not_equal_to(prev, this.data_.end()) && prev.second === it.second) this.data_.erase(it);
        if (next.equals(this.data_.end()) === true || next.first !== index + 1 || next.second !== val) {
    ...
    _Insert_by_repeating_val(pos, n, val) {
        ...
        if (pos.equals(this.end()) === true) return this._Insert_to_end(elements); else return this._Insert_to_middle(pos, elements);
    }
    _Insert_by_range(pos, first, last) {
        const elements = [];
        for (let it = first; !it.equals(last); it = it.next()) {
            if (elements.length === 0 || elements[elements.length - 1].second !== it.value) elements.push(new Pair(1, it.value)); else ++elements[elements.length - 1].first;
        }
        if (pos.equals(this.end()) === true) return this._Insert_to_end(elements); else return this._Insert_to_middle(pos, elements);
    }
...
class ForwardList {
    ...
    insert_after(pos, ...args) {
        let ret;
        if (args.length === 1) ret = this._Insert_by_repeating_val(pos, 1, args[0]); else if (typeof args[0] === "number") ret = this._Insert_by_repeating_val(pos, args[0], args[1]); else ret = this._Insert_by_range(pos, args[0], args[1]);
        return ret;
    }
    _Insert_by_repeating_val(pos, n, val) {
        const first = new Repeater(0, val);
    ...
    _Insert_by_range(pos, first, last) {
        ...
        for (let i = 0; i < count - 1; ++i) ForwardList.Iterator._Set_next(nodes[i], nodes[i + 1]);
    ...
    remove_if(pred) {
        let count = 0;
        for (let it = this.before_begin(); !it.next().equals(this.end()); it = it.next()) if (pred(it.next().value) === true) {
    ...
    merge(from, comp = less) {
        ...
        while (from.empty() === false) {
            ...
            while (!it.next().equals(this.end()) && comp(it.next().value, value)) it = it.next();
            this.splice_after(it, from, from.before_begin());
        }
    }
    splice_after(pos, from, first_before = from.before_begin(), last = first_before.next().next()) {
    ...
    class Iterator {
        ...
        _Try_value() {
            if (this.value_ === undefined) {
                ...
                if (this.equals(source.end()) === true) throw ErrorGenerator.iterator_end_value(source); else if (this.equals(source.before_begin()) === true) throw ErrorGenerator.iterator_end_value(source, "before_begin");
            }
        }
        next() {
            return this.next_;
...
class SetTree extends XTree {
    ...
    lower_bound(val) {
        ...
        if (node === null) return this.source_.end(); else if (this.key_comp_(node.value.value, val)) return node.value.next(); else return node.value;
    }
    equal_range(val) {
...
class UniqueSetTree extends SetTree {
    ...
    nearest_by_key(val) {
        ...
        while (true) {
            const it = ret.value;
            let my_node = null;
            if (this.key_comp()(val, it.value)) my_node = ret.left; else if (this.key_comp()(it.value, val)) my_node = ret.right; else return ret;
            if (my_node === null) break; else ret = my_node;
        }
        return ret;
    }
...
class TreeSet extends UniqueTreeSet {
    constructor(...args) {
        ...
        ITreeContainer.construct(this, TreeSet, (comp => {
...
class HashBuckets {
    ...
    rehash(length) {
        ...
        for (const row of this.data_) for (const elem of row) {
    ...
    max_load_factor(z = null) {
        if (z === null) return this.max_load_factor_; else this.max_load_factor_ = z;
    }
    hash_function() {
        return this.hasher_;
    }
    ...
    erase(val) {
        ...
        for (let i = 0; i < bucket.length; ++i) if (bucket[i] === val) {
            bucket.splice(i, 1);
            --this.size_;
            break;
        }
...
class HashSet extends UniqueSet {
    constructor(...args) {
        super((thisArg => new SetElementList(thisArg)));
        IHashContainer.construct(this, HashSet, ((hash, pred) => {
    ...
    begin(index = null) {
        if (index === null) return super.begin(); else return this.buckets_.at(index)[0];
    }
    end(index = null) {
        if (index === null) return super.end(); else {
    ...
    _Handle_insert(first, last) {
        for (;!first.equals(last); first = first.next()) this.buckets_.insert(first);
    }
    _Handle_erase(first, last) {
        for (;!first.equals(last); first = first.next()) this.buckets_.erase(first);
    }
}
...
class MultiSetTree extends SetTree {
    constructor(source, comp) {
        super(source, comp, (function(x, y) {
            ...
            if (!ret && !comp(y.value, x.value)) return get_uid(x) < get_uid(y); else return ret;
        }));
    }
    insert(val) {
        get_uid(val);
    ...
    _Nearest_by_key(key, equal_mover) {
        ...
        while (true) {
            ...
            let node = null;
            if (this.key_comp()(key, candidate.value)) node = ret.left; else if (this.key_comp()(candidate.value, key)) node = ret.right; else {
                matched = ret;
                node = equal_mover(ret);
            }
            if (node === null) break; else ret = node;
        }
...
class TreeMultiSet extends MultiTreeSet {
    constructor(...args) {
        super((thisArg => new SetElementList(thisArg)));
        ITreeContainer.construct(this, TreeMultiSet, (comp => {
...
class HashMultiSet extends MultiSet {
    constructor(...args) {
        super((thisArg => new SetElementList(thisArg)));
        IHashContainer.construct(this, HashMultiSet, ((hash, pred) => {
            this.buckets_ = new SetHashBuckets(this, hash, pred);
        }), ...args);
    }
    clear() {
    ...
    _Insert_by_range(first, last) {
        ...
        if (this.size() > this.buckets_.capacity()) this.reserve(Math.max(this.size(), this.buckets_.capacity() * 2));
...
class HashMap extends UniqueMap {
    constructor(...args) {
        super((thisArg => new MapElementList(thisArg)));
        IHashContainer.construct(this, HashMap, ((hash, pred) => {
...
class MultiMapTree extends MapTree {
    constructor(source, comp) {
        super(source, comp, (function(x, y) {
            ...
            if (!ret && !comp(y.first, x.first)) return get_uid(x) < get_uid(y); else return ret;
        }));
    }
    insert(val) {
        get_uid(val);
    ...
    _Nearest_by_key(key, equal_mover) {
        ...
        while (true) {
            const it = ret.value;
            let my_node = null;
            if (this.key_comp()(key, it.first)) my_node = ret.left; else if (this.key_comp()(it.first, key)) my_node = ret.right; else {
                matched = ret;
...
class TreeMultiMap extends MultiTreeMap {
    constructor(...args) {
        super((thisArg => new MapElementList(thisArg)));
        ITreeContainer.construct(this, TreeMultiMap, (comp => {
...
class HashMultiMap extends MultiMap {
    constructor(...args) {
        super((thisArg => new MapElementList(thisArg)));
        IHashContainer.construct(this, HashMultiMap, ((hash, pred) => {
            this.buckets_ = new MapHashBuckets(this, hash, pred);
        }), ...args);
    }
    clear() {
    ...
    _Insert_by_range(first, last) {
        const entries = [];
        for (let it = first; !it.equals(last); it = it.next()) entries.push(new Entry(it.value.first, it.value.second));
        const my_first = this.data_.insert(this.data_.end(), new NativeArrayIterator(entries, 0), new NativeArrayIterator(entries, entries.length));
        if (this.size() > this.buckets_.capacity()) this.reserve(Math.max(this.size(), this.buckets_.capacity() * 2));
        this._Handle_insert(my_first, this.end());
    }
    _Handle_insert(first, last) {
...
class Stack extends AdaptorContainer {
    constructor(obj) {
        super(new Vector);
        if (obj !== undefined) this.source_.assign(obj.source_.begin(), obj.source_.end());
    }
    top() {
...
class ErrorInstance {
    ...
    toJSON() {
        if (this.category_ === null) return {}; else return {
...
function tgamma(x) {
    if (x < .5) return Math.PI / (Math.sin(Math.PI * x) * tgamma(1 - x));
    x -= 1;
    let a = P[0];
    const t = x + G$1 + .5;
...
const P = [ .9999999999998099, 676.5203681218851, -1259.1392167224028, 771.3234287776531, -176.6150291621406, 12.507343278686905, -.13857109526572012, 9984369578019572e-21, 1.5056327351493116e-7 ];
...
(function(MathUtil) {
    function factorial(k) {
        if (FACTORIALS.length <= k) for (let i = FACTORIALS.length; i <= k; ++i) FACTORIALS.push(FACTORIALS[i - 1] * i);
        return FACTORIALS[k];
    }
    MathUtil.factorial = factorial;
    function integral(formula, first, last, segment_count = 100 * 1e3) {
        if (first > last) [first, last] = [ last, first ]; else if (first === last) return 0;
...
function cyl_bessel_j(n, x) {
    if (x < 0 && Math.floor(n) !== n) throw new InvalidArgument(`Error on std.cyl_bessel_j(): n must be integer when x is negative -> (n = ${n}, x = ${x}).`); else if (x === 0 && n !== 0) throw new InvalidArgument(`Error on std.cyl_bessel_j(): n must be zero when x is zero -> (n = ${n}, x = ${x}).`);
    if (n === Math.floor(n)) return _J_int(n, x); else return _J_positive(n, x);
}

function cyl_neumann(v, x) {
    if (x <= 0) throw new InvalidArgument(`Error on std.cyl_neumann(): x must be greater than zero -> (x = ${x}).`);
    const numerator = cyl_bessel_j(v, x) * Math.cos(v * Math.PI) - cyl_bessel_j(-v, x);
    const denominator = Math.sin(v * Math.PI);
    return numerator / denominator;
}

function _J_int(n, x) {
    if (n < 0) return Math.pow(-1, n) * _J_positive(-n, x); else return _J_positive(n, x);
}

function _J_positive(v, x) {
    const sigma = MathUtil.sigma((function(k) {
...
function cyl_bessel_i(n, x) {
    if (x < 0 && Math.floor(n) !== n) throw new InvalidArgument(`Error on std.cyl_bessel_i(): n must be integer when x is negative -> (n = ${n}, x = ${x}).`); else if (x === 0 && n !== 0) throw new InvalidArgument(`Error on std.cyl_bessel_i(): n must be zero when x is zero -> (n = ${n}, x = ${x}).`);
    if (n === .5) return Math.sqrt(2 / (Math.PI * x)) * Math.sinh(x); else return _Bessel_i(n, x);
}

function _Bessel_i(v, x) {
    return MathUtil.sigma((function(k) {
...
function cyl_bessel_k(n, x) {
    if (x <= 0) throw new InvalidArgument(`Error on std.cyl_bessel_k(): requires x > 0 -> (x = ${x}).`);
...
function ellint_3(k, v, phi) {
    ...
    if (v > predicator) throw new InvalidArgument(`Error on std.ellint_3(): must be v < (1 / sin^2(phi)) -> (v = ${v}, 1 / sin^2(phi) = ${predicator}).`);
...
function _Post_process(func, k, phi, formula) {
    if (Math.abs(k) > 1) throw new InvalidArgument(`Error on std.${func}(): must be |k| <= 1 -> (k = ${k}).`);
    const area = MathUtil.integral(formula, 0, phi);
    return phi < 0 ? -area : area;
}

function expint(x) {
    if (x === 0) return -Infinity; else if (x < 0) return -_E1_G(-x); else return _EI_Factorial(x);
}

function _EI_Factorial(x) {
    return EULER + Math.log(Math.abs(x)) / Math.log(Math.E) + MathUtil.sigma((function(k) {
...
const H_INF = (1 - G) * (G * G - 6 * G + 12) / (3 * G * Math.pow(2 - G, 2) * B);

function hermite(n, x) {
    if ((n = Math.floor(n)) < 0) throw new InvalidArgument(`Error on std.hermite(): n must be unsigned integer -> (n = ${n}).`);
...
function assoc_legendre(n, m, x) {
    if ((n = Math.floor(n)) < 0 || (m = Math.floor(m)) < 0) throw new InvalidArgument(`Error on std.assoc_legendre(): both n and m must be unsigned integer -> (n = ${n}, m = ${m}).`); else if (Math.abs(x) > 1) throw new InvalidArgument(`Error on std.assoc_legendre(): must be |x| <= 1 -> (x = ${x}).`);
    const matrix = [ [ 1, x ] ];
    matrix.length = m + 1;
...
function _Compute_assoc_legendre(n, m, x, matrix) {
    if (n < 0) n = -n - 1;
    if (m === 0) return _Compute_legendre(n, x, matrix[0]); else if (matrix[m].length > n && matrix[m][n] !== undefined) return matrix[m][n];
    const left = (n - m + 1) * (n - m + 2) * _Compute_assoc_legendre(n + 1, m - 1, x, matrix);
    const right = (n + m - 1) * (n + m) * _Compute_assoc_legendre(n - 1, m - 1, x, matrix);
...
function assoc_laguerre(n, m, x) {
    if ((n = Math.floor(n)) < 0 || (m = Math.floor(m)) < 0) throw new InvalidArgument(`Error on std.assoc_laguerre(): both n and m must be unsigned integer -> (n = ${n}, m = ${m}).`);
...
function riemann_zeta(arg) {
    if (arg < 0) return _Negative(arg); else if (arg === 0) return -.5; else if (arg < 1) return _Fractional(arg); else if (arg === 1) return Infinity; else return _Positive(arg);
}

function _Negative(arg) {
    return Math.pow(2, arg) * Math.pow(Math.PI, arg - 1) * Math.sin(Math.PI * arg / 2) * tgamma(1 - arg) * riemann_zeta(1 - arg);
}

function _Fractional(arg) {
    const divider = 1 - Math.pow(2, 1 - arg);
    const sigma = MathUtil.sigma((function(n) {
...
function _Positive(arg) {
    return MathUtil.sigma((function(n) {
...
async function try_lock(...items) {
    for (let i = 0; i < items.length; ++i) if (await items[i].try_lock() === false) return i;
    return -1;
}
...
class SharedTimedMutex {
    ...
    try_lock_for(ms) {
        return new Promise((resolve => {
            ...
            if (it.value.handler === null) resolve(true); else {
                sleep_for(ms).then((() => {
    ...
    async unlock() {
        if (this._Current_access_type() !== 0) throw new InvalidArgument(`Error on std.${this.source_.constructor.name}.unlock(): this mutex is free on the unique lock.`);
        --this.writing_;
    ...
    async unlock_shared() {
        if (this._Current_access_type() !== 1) throw new InvalidArgument(`Error on std.${this.source_.constructor.name}.unlock_shared(): this mutex is free on the shared lock.`);
        --this.reading_;
        this.queue_.pop_front();
        this._Release();
    }
    _Release() {
        ...
        for (const resolver of this.queue_) {
            if (resolver.accessType !== current) break; else if (resolver.handler !== null) {
                resolverList.push({
                    ...resolver
                });
        ...
        for (const resolver of resolverList) if (resolver.lockType === 0) resolver.handler(); else resolver.handler(true);
    }
    _Cancel(it) {
        ...
        const prev = it.prev();
        if (prev.equals(this.queue_.end()) === false && prev.value.handler === null) this._Release();
        handler(false);
    }
}
...
class ConditionVariable {
    ...
    async wait_until(at, predicator) {
        ...
        while (!await predicator()) if (!await this._Wait_until(at)) return await predicator();
        return true;
    }
    _Wait() {
        return new Promise((resolve => {
    ...
    async notify_one() {
        ...
        this.resolvers_.erase(it);
        if (it.value.lockType === 0) it.value.handler(); else it.value.handler(true);
    }
    async notify_all() {
        if (this.resolvers_.empty()) return;
...
(function(UniqueLock) {
    function lock(mutex, closure) {
        return SafeLock.lock((() => mutex.lock()), (() => mutex.unlock()), closure);
    }
    UniqueLock.lock = lock;
    function try_lock(mutex, closure) {
        return SafeLock.try_lock((() => mutex.try_lock()), (() => mutex.unlock()), closure);
    }
    UniqueLock.try_lock = try_lock;
    function try_lock_for(mutex, ms, closure) {
        return SafeLock.try_lock((() => mutex.try_lock_for(ms)), (() => mutex.unlock()), closure);
    }
    UniqueLock.try_lock_for = try_lock_for;
    function try_lock_until(mutex, at, closure) {
        return SafeLock.try_lock((() => mutex.try_lock_until(at)), (() => mutex.unlock()), closure);
    }
...
(function(SharedLock) {
    function lock(mutex, closure) {
        return SafeLock.lock((() => mutex.lock_shared()), (() => mutex.unlock_shared()), closure);
    }
    SharedLock.lock = lock;
    function try_lock(mutex, closure) {
        return SafeLock.try_lock((() => mutex.try_lock_shared()), (() => mutex.unlock_shared()), closure);
    }
    SharedLock.try_lock = try_lock;
    function try_lock_for(mutex, ms, closure) {
        return SafeLock.try_lock((() => mutex.try_lock_shared_for(ms)), (() => mutex.unlock_shared()), closure);
    }
    SharedLock.try_lock_for = try_lock_for;
    function try_lock_until(mutex, at, closure) {
        return SafeLock.try_lock((() => mutex.try_lock_shared_until(at)), (() => mutex.unlock_shared()), closure);
    }
...
class Semaphore {
    ...
    async release(n = 1) {
        if (n < 1) throw new InvalidArgument(`Error on std.Semaphore.release(): parametric n is less than 1 -> (n = ${n}).`); else if (n > this.max_) throw new OutOfRange(`Error on std.Semaphore.release(): parametric n is greater than max -> (n = ${n}, max = ${this.max_}).`); else if (n > this.acquiring_) throw new OutOfRange(`Error on std.Semaphore.release(): parametric n is greater than acquiring -> (n = ${n}, acquiring = ${this.acquiring_}).`);
        const resolverList = [];
        while (this.queue_.empty() === false && resolverList.length < n) {
            const resolver = this.queue_.front();
            if (resolver.handler !== null) resolverList.push({
                ...resolver
            });
            this.queue_.pop_front();
            resolver.handler = null;
...
class Latch {
    ...
    async wait_for(ms) {
        if (this._Try_wait() === true) return true; else return await this.cv_.wait_for(ms);
    }
    async wait_until(at) {
        if (this._Try_wait() === true) return true; else return await this.cv_.wait_until(at);
    }
    _Try_wait() {
...
class MutableSingleton {
    ...
    async get(...args) {
        ...
        if (output === NOT_MOUNTED_YET$1) await UniqueLock.lock(this.mutex_, (async () => {
...
class VariadicMutableSingleton {
    ...
    async clear(...args) {
        if (args.length === 0) this.dict_.clear(); else await this._Get_singleton(args).clear();
    }
    get(...args) {
    ...
    _Get_singleton(args) {
        ...
        if (it.equals(this.dict_.end()) === true) it = this.dict_.emplace(args, new MutableSingleton(this.closure_)).first;
        return it.second;
    }
}

class VariadicTimedSingleton {
    constructor(interval, closure, hasher = (args => hash(...args)), pred = equal) {
        this.interval_ = interval;
        this.closure_ = closure;
...
class VariadicSingleton {
    ...
    get(...args) {
        let it = this.dict_.find(args);
        if (it.equals(this.dict_.end()) == true) it = this.dict_.emplace(args, new Singleton(this.closure_)).first;
        return it.second.get(...args);
    }
}
...
export { BackInsertIterator, Barrier, ConditionVariable, Deque, DomainError, Entry, ErrorCategory, ErrorCode, ErrorCondition, Exception, ForwardList, FrontInsertIterator, HashMap, HashMultiMap, HashMultiSet, HashSet, InsertIterator, InvalidArgument, Latch, LengthError, List, LogicError, MutableSingleton, Mutex, OutOfRange, OverflowError, Pair, PriorityQueue, Queue, RangeError, RuntimeError, Semaphore, SharedLock, SharedMutex, SharedTimedMutex, Singleton, Stack, SystemError, TimedMutex, TimedSingleton, TreeMap, TreeMultiMap, TreeMultiSet, TreeSet, UnderflowError, UniqueLock, VariadicMutableSingleton, VariadicSingleton, VariadicTimedSingleton, Vector, VectorBoolean, accumulate$1 as accumulate, adjacent_difference$1 as adjacent_difference, adjacent_find$1 as adjacent_find, advance, all_of$1 as all_of, any_of$1 as any_of, assoc_laguerre, assoc_legendre, back_inserter, module$2 as base, begin, beta, binary_search$1 as binary_search, bit_and, bit_or, bit_xor, clamp, comp_ellint_1, comp_ellint_2, comp_ellint_3, copy$1 as copy, copy_backward$1 as copy_backward, copy_if$1 as copy_if, copy_n$1 as copy_n, count$1 as count, count_if$1 as count_if, cyl_bessel_i, cyl_bessel_j, cyl_bessel_k, cyl_neumann, std as default, distance, divides, ellint_1, ellint_2, ellint_3, empty, end, equal$1 as equal, equal_range$1 as equal_range, equal_to, exclusive_scan$1 as exclusive_scan, module$1 as experimental, expint, fill$1 as fill, fill_n$1 as fill_n, find$1 as find, find_end$1 as find_end, find_first_of$1 as find_first_of, find_if$1 as find_if, find_if_not$1 as find_if_not, for_each$1 as for_each, for_each_n$1 as for_each_n, front_inserter, gcd, generate$1 as generate, generate_n$1 as generate_n, get_uid, greater, greater_equal, hash, hermite, includes$1 as includes, inclusive_scan$1 as inclusive_scan, inner_product$1 as inner_product, inplace_merge$1 as inplace_merge, inserter, iota$1 as iota, is_heap$1 as is_heap, is_heap_until$1 as is_heap_until, is_node, is_partitioned$1 as is_partitioned, is_permutation$1 as is_permutation, is_sorted$1 as is_sorted, is_sorted_until$1 as is_sorted_until, is_unique$1 as is_unique, iter_swap, laguerre, lcm, legendre, less, less_equal, lexicographical_compare$1 as lexicographical_compare, lgamma, lock, logical_and, logical_not, logical_or, lower_bound$1 as lower_bound, make_heap$1 as make_heap, make_pair, make_reverse_iterator, max, max_element$1 as max_element, merge$1 as merge, min, min_element$1 as min_element, minmax, minmax_element$1 as minmax_element, minus, mismatch$1 as mismatch, modules, multiplies, negate, next, next_permutation$1 as next_permutation, none_of$1 as none_of, not_equal_to, nth_element$1 as nth_element, partial_sort$1 as partial_sort, partial_sort_copy$1 as partial_sort_copy, partial_sum$1 as partial_sum, partition$1 as partition, partition_copy$1 as partition_copy, partition_point$1 as partition_point, plus, pop_heap$1 as pop_heap, prev, prev_permutation$1 as prev_permutation, push_heap$1 as push_heap, randint, module as ranges, rbegin, remove$1 as remove, remove_copy$1 as remove_copy, remove_copy_if$1 as remove_copy_if, remove_if$1 as remove_if, rend, replace$1 as replace, replace_copy$1 as replace_copy, replace_copy_if$1 as replace_copy_if, replace_if$1 as replace_if, reverse$1 as reverse, reverse_copy$1 as reverse_copy, riemann_zeta, rotate$1 as rotate, rotate_copy$1 as rotate_copy, sample$1 as sample, search$1 as search, search_n$1 as search_n, set_difference$1 as set_difference, set_intersection$1 as set_intersection, set_symmetric_difference$1 as set_symmetric_difference, set_union$1 as set_union, shift_left$1 as shift_left, shift_right$1 as shift_right, shuffle$1 as shuffle, size, sleep_for, sleep_until, sort$1 as sort, sort_heap$1 as sort_heap, sph_bessel, sph_neumann, stable_partition$1 as stable_partition, stable_sort$1 as stable_sort, swap_ranges$1 as swap_ranges, tgamma, transform$1 as transform, transform_exclusive_scan$1 as transform_exclusive_scan, transform_inclusive_scan$1 as transform_inclusive_scan, try_lock, unique$1 as unique, unique_copy$1 as unique_copy, upper_bound$1 as upper_bound };
